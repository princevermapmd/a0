...
$f_size = round($file['size']/1024/1024,2); //2mb size related
...
$user_id_folder ="user_".$user_data['id']; //user find check
...
$free_space = $total_storage-$old_used_storage;
    ...
    {
        echo json_encode(array("msg"=>"file already exited"));  
    ...
    else
    {
       if(move_uploaded_file($location,"../data/".$user_id_folder."/".$filename))
            {
                $file_store ="INSERT INTO .$user_id_folder(
                file_name,file_size)VALUES('$filename','$f_size')";
                ...
                if($db->query($file_store))
                {
                   ...
                   $update ="UPDATE users SET used_storage ='$total_used_file_size' WHERE email ='$user_email'";
                    ...
                    if($db->query($update))
                    {
                        echo json_encode(array("msg"=>"File Upload Succesfully","used_storage"
